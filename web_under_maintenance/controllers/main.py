...
class WebSettingsDashboard(WSD):
    ...
    def web_settings_dashboard_data(self, **kw):
        ...
        if 'share' in result:
            result["share"]["under_maintenance"] = bool(int(ir_param.get_param('under_maintenance')))
...
class WebUnderMaintenance(http.Controller):
    ...
    def web_under_maintenance(self, *args, **kwargs):
        ...
        session_obj = request.env['ir.session'].sudo()
        # Toggle mode
        under_maintenance = 0 if bool(int(ir_param.get_param('under_maintenance'))) else 1
        ir_param.set_param('under_maintenance', str(under_maintenance))
